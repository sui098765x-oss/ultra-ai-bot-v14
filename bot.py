...
ADMIN_ID = int(os.environ.get("ADMIN_ID", "123456789"))  # নিজের Telegram ID দাও
...
cur = db.cursor()
cur.execute("CREATE TABLE IF NOT EXISTS users(user_id INTEGER PRIMARY KEY, mode TEXT)")
db.commit()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🚀 Ultra AI Bot v14 Premium Ready!",
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if mode == "ai":
            res = requests.get(f"https://text.pollinations.ai/{text}").text
            ...
            voice = "bn-BD-NabanitaNeural" if any("\u0980" <= c <= "\u09FF" for c in text) else "en-US-AvaNeural"
            ...
            if "data" in res:
                await update.message.reply_video(res["data"]["play"])
            else:
                await update.message.reply_text("❌ ভিডিও পাওয়া যায়নি")

        elif mode == "safe":
            if text.startswith("http"):
                await update.message.reply_text("🔍 লিংক চেক করা হয়েছে। সাবধান থাকুন।")
            else:
                await update.message.reply_text("⚠️ সঠিক লিংক দিন")

        elif mode == "sum":
            res = requests.get(f"https://text.pollinations.ai/Summarize in Bengali: {text}").text
            await update.message.reply_text(res)
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if get_mode(user_id) == "pdf":
        ...
        path = f"{user_id}_{len(context.user_data['images'])}.jpg"
...
async def create_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pdf = f"{update.message.from_user.id}.pdf"
...
async def main():
    ...
    print("🚀 Ultra AI Bot v14 Running...")
